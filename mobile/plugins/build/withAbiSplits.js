...
const withAbiSplits = (config) => {
    return (0, config_plugins_1.withAppBuildGradle)(config, (config) => {
        ...
        const obsoleteExcludes = 'excludes += ["**/armeabi/*.so", "**/mips/*.so", "**/mips64/*.so", "**/x86/*.so"]';
...
`;
        ...
        contents = contents.replace(/enable\s+(true|false|!project\.hasProperty\("ucUniversalOnly"\))/, 'enable !project.hasProperty("ucUniversalOnly")');
        config.modResults.contents = contents.replace(/universalApk\s+(true|false)/, 'universalApk false');
        return config;
    });
};
exports.default = (0, config_plugins_1.createRunOncePlugin)(withAbiSplits, 'withAbiSplits', '3.0.0');
