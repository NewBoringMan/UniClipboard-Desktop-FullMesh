...
const androidReleaseBuildScript = readFileSync(
  join(root, 'scripts', 'build-android-release.sh'),
  'utf8'
);
const androidReleaseVerifyScript = readFileSync(
  join(root, 'scripts', 'verify-android-release.sh'),
  'utf8'
);
const androidEngineBuild = readFileSync(
  join(root, 'modules', 'uc-engine', 'android', 'build.gradle'),
  'utf8'
);
...
describe('validated release workflow', () => {
  ...
  it('publishes every Android ABI and the universal package required by FullMesh', () => {
    for (const releaseSurface of [androidBuildWorkflow, androidManifestScript]) {
      expect(releaseSurface).toContain('arm64-v8a');
      expect(releaseSurface).toContain('armeabi-v7a');
      expect(releaseSurface).toContain('x86_64');
      expect(releaseSurface).toContain('universal');
    }
    expect(releaseWorkflow).toContain('android-installers');
    expect(releaseWorkflow).toContain('artifacts: android-installers/*');
    ...
    expect(androidReleaseVerifyScript).toContain('UC_ENGINE_EXPECTED_AAR');
    expect(androidReleaseVerifyScript).toContain('Engine payload mismatch');
    expect(androidBuildWorkflow).toContain('.aab');
    expect(androidBuildWorkflow).toContain('if-no-files-found: error');
  });

  it('can consume an exact locally built Engine candidate without changing the public pin', () => {
    expect(androidEngineBuild).toContain("System.getenv('UC_ENGINE_LOCAL_MAVEN')");
    expect(androidEngineBuild).toContain("System.getenv('UC_ENGINE_LOCAL_VERSION')");
    expect(androidEngineBuild).toContain('resolvedEngineVersion');
    expect(androidEngineBuild).toContain(
