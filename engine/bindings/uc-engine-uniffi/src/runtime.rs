...
impl MobileEngine {
    fn start_inner(
        ...
        mailbox: Option<crate::BindingMailboxConfig>,
        ...
        let config = if let Some(mailbox) = mailbox {
            config.with_mailbox_service(
                mailbox.base_url,
                uc_engine::SecretString::new(mailbox.bearer_token),
            )
        } else {
            config
        };
...
impl MobileEngine {
    ...
    pub fn start_with_mailbox(
        config: BindingConfig,
        host: Arc<dyn BindingHost>,
        mailbox: crate::BindingMailboxConfig,
    ) -> Result<Arc<Self>, BindingError> {
        Self::start_inner(config, host, None, Some(mailbox))
    }

    #[uniffi::constructor]
    pub fn start_with_analytics_and_mailbox(
        config: BindingConfig,
        host: Arc<dyn BindingHost>,
        analytics: Arc<dyn BindingAnalyticsHost>,
        context: BindingAnalyticsContext,
        ...
        Self::start_inner(config, host, Some((analytics, context)), Some(mailbox))
    }
