...
impl OhEngine {
    pub(crate) async fn start(config: OhEngineConfig, host: OhHost) -> napi::Result<Self> {
        ...
        let mailbox = config.mailbox_service_url.zip(config.mailbox_bearer_token);
        let config = EngineConfig::new(config.app_version).with_profile_id(config.profile_id);
        let config = if let Some((url, token)) = mailbox {
            config.with_mailbox_service(url, uc_engine::SecretString::new(token))
        } else {
            config
        };
