...
use uc_core::ports::{
    ClipboardDeliveryTransport, ClipboardReceiverPort, ClockPort, InboundClipboard,
...
struct PreparedInbound {
    ...
    flow: InboundFlow,
    at_ms: i64,
    transport: ClipboardDeliveryTransport,
...
impl InboundProcessor {
    ...
    async fn handle_one(&self, inbound: InboundClipboard) {
        ...
        let (action, disposition) = match &result {
            ...
            Ok(
                ...
                | InboundClipboardApplyOutcome::ReplaySkipped { .. },
            ) => {
    ...
    async fn prepare(
        ...
        let event_id = match inbound.header.event.as_ref() {
            Some(event)
                if event.origin_device_id != inbound.peer_device_id
                    || event.content_hash != inbound.header.snapshot_hash
                    || event.is_expired_at(self.clock.now_ms()) =>
            {
                warn!(
                    peer = %inbound.peer_device_id.as_str(),
                    error_kind = "inbound_fullmesh_event_rejected",
                    "inbound FullMesh event metadata rejected"
                );
                receipt.finish(InboundClipboardDisposition::Rejected);
                return None;
            }
            Some(event) => Some(event.event_id.clone()),
            None => None,
        };
...
fn transport_type_from_channel(channel: ClipboardDeliveryTransport) -> TransportType {
    match channel {
        ClipboardDeliveryTransport::Direct => TransportType::P2pDirect,
        ClipboardDeliveryTransport::Relay => TransportType::Relay,
        ClipboardDeliveryTransport::Mailbox => TransportType::Mailbox,
        ClipboardDeliveryTransport::Unknown => TransportType::Unknown,
    }
}
...
mod tests {
    ...
    use uc_core::ports::{
        ClipboardDeliveryTransport, ClipboardHeader, ClipboardReceiverPort, ClockPort,
        InboundClipboard, InboundClipboardDisposition, InboundClipboardReceipt,
        InboundClipboardResult, SettingsPort,
    };
    ...
    fn fixture_with_ciphertext(
        ...
        (
            InboundClipboard {
                ...
                ciphertext,
                transport: ClipboardDeliveryTransport::Unknown,
    ...
    async fn runtime_writes_relay_stage_timings_to_the_otlp_log_after_receiver_applies() {
        ...
        inbound.transport = ClipboardDeliveryTransport::Relay;
