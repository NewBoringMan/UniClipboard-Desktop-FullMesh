...
use uc_core::ports::{
    ClipboardEventRepositoryPort, ClipboardHeader, ClockPort, LocalIdentityPort, SettingsPort,
};
...
impl OutboundHeaderFactory {
    ...
    ) -> anyhow::Result<ClipboardHeader> {
        let origin_device_name = self.load_origin_device_name().await;
        // A resend is a new delivery event even when the underlying history
        // entry is old.  Stamp it from the dispatch clock so its bounded TTL
        // does not make an explicit resend expire before it reaches a peer.
        let created_at_ms = self.clock.now_ms().max(0);
        let mime_representations = if input.mime_representations.is_empty() {
            vec![ClipboardMimeRepresentation {
                mime_type: "application/octet-stream".to_owned(),
                size_bytes: u64::try_from(input.plaintext.len()).unwrap_or(u64::MAX),
            }]
        } else {
            input.mime_representations.clone()
        };
        let origin_sequence = match self.event_repo.as_ref() {
            Some(repository) => {
                repository
                    .allocate_origin_sequence(local_device, created_at_ms)
                    .await?
            }
            None => uc_core::clipboard::next_origin_sequence(created_at_ms),
        };
        let event = ClipboardEventEnvelope::new_with_sequence(
            *local_device,
            origin_sequence,
            created_at_ms,
            input.snapshot_hash.clone(),
            mime_representations,
            input.payload_references.clone(),
            input.target_filter.clone().unwrap_or_default(),
        );
        Ok(ClipboardHeader {
...
mod tests {
    ...
    async fn build_stamps_header_and_uses_settings_device_name() {
        ...
        let header = factory
            .build(&input, &flow, &dev("self-device"))
            .await
            .expect("header build");
        ...
        let event = header.event.expect("FullMesh header must carry an event");
        assert_eq!(event.origin_device_id.as_str(), "self-device");
        assert_eq!(event.content_hash, input.snapshot_hash);
        assert!(event.origin_sequence > 0);
    }
