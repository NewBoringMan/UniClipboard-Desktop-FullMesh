...
use crate::clipboard::inbound::mailbox::MailboxFrameV1;
use crate::facade::blob_transfer::SharedHostEventEmitter;
use uc_core::clipboard::{
    ClipboardContentCategory, ClipboardContentCategorySet, ClipboardMimeRepresentation,
    ClipboardPayloadReference, EntryDeliveryRecord,
};
use uc_core::ids::{DeviceId, EntryId};
use uc_core::membership::ContentExchangeGatePort;
use uc_core::ports::security::TransferCipherPort;
use uc_core::ports::{
    ClipboardDispatchError, ClipboardDispatchPort, ClipboardMailboxPort, ClockPort,
    ConnectionChannel, DeviceIdentityPort, DispatchAck, EntryDeliveryRepositoryPort,
...
pub(crate) struct DispatchClipboardEntryUseCase {
    ...
    mailbox: Option<Arc<dyn ClipboardMailboxPort>>,
}
...
impl DispatchClipboardEntryUseCase {
    ...
    pub(crate) fn with_mailbox(mut self, mailbox: Arc<dyn ClipboardMailboxPort>) -> Self {
        self.mailbox = Some(mailbox);
        self
    }

    pub(crate) fn with_event_repository(
        mut self,
        event_repo: Arc<dyn uc_core::ports::ClipboardEventRepositoryPort>,
    ) -> Self {
        self.header_factory = self.header_factory.with_event_repository(event_repo);
        self
    }

    async fn spawn_mailbox_race(
        &self,
        local_device: &DeviceId,
        candidates: &[DeviceId],
        categories: &ClipboardContentCategorySet,
        header: &uc_core::ports::ClipboardHeader,
        ...
        if categories
            .iter()
            .any(|category| category == ClipboardContentCategory::File)
        {
            return;
        }
        let Some(event) = header.event.as_ref() else {
            warn!("clipboard mailbox skipped a legacy header without event identity");
            return;
        };
        let package = match MailboxFrameV1::encode(local_device, header, inner_ciphertext) {
            Ok(package) => package,
            Err(error) => {
                warn!(error = %error, "clipboard mailbox frame encoding failed");
                return;
            }
        };
        let outer_ciphertext = match self.cipher.encrypt(&package).await {
            Ok(bytes) => Bytes::from(bytes),
            Err(error) => {
                warn!(error = %error, "clipboard mailbox outer encryption failed");
                return;
            }
        };
        let ttl_ms = event.expires_at_ms.saturating_sub(event.created_at_ms);
        let ttl_seconds = u32::try_from((ttl_ms / 1_000).max(30)).unwrap_or(86_400);
        let event_id = event.event_id.clone();
        for target in candidates.iter().copied() {
            let mailbox = Arc::clone(mailbox);
            let event_id = event_id.clone();
            let ciphertext = outer_ciphertext.clone();
            uc_observability_contract::spawn_supervised(
                "clipboard_sync.mailbox_upload",
                async move {
                    match mailbox
                        .upload(&target, &event_id, ttl_seconds, ciphertext)
                        .await
                    {
                        Ok(outcome) => {
                            info!(peer.device_id = %target.as_str(), outcome = ?outcome, "clipboard mailbox upload settled");
                        }
                        Err(error) => {
                            warn!(peer.device_id = %target.as_str(), error = %error, "clipboard mailbox upload failed; live route remains active");
                        }
                    }
                },
            );
    ...
    pub(crate) async fn execute(
        ...
        let header = self
            ...
            .await
            .map_err(|error| DispatchSyncError::Repository(error.to_string()))?;
        ...
        // Small text/image payloads race the live QUIC path with a bounded
        // encrypted mailbox upload. Both carry the exact same EventId, so the
        // receiver's persistent event window decides the winner. File-class
        // payloads remain direct/relay-first and use the resumable blob path.
        self.spawn_mailbox_race(
            &local_device,
            &candidates,
            &input.categories,
            &header,
...
mod tests {
    ...
    fn input() -> DispatchClipboardEntryInput {
        DispatchClipboardEntryInput {
            ...
            mime_representations: vec![],
            payload_references: vec![],
        }
    }
