...
use bytes::Bytes;
use thiserror::Error;
use uc_core::ids::{DeviceId, EntryId, EventId};
...
pub struct ApplyInboundInput {
    ...
    /// FullMesh event identity. Legacy v1-v3 senders leave it empty and use
    /// the existing persistent content-hash compatibility path.
    ...
    ///   desktops.
    ///
    /// [`ClipboardWriteIntent`]: crate::clipboard::write::ClipboardWriteIntent
...
pub enum ApplyOutcome {
    ...
    /// The EventId is still inside the persistent replay window even though
    /// the user-visible history row has already been deleted. No payload is
    /// decoded, persisted or written back to the system clipboard.
    ReplaySkipped { event_id: EventId },
