...
use uc_core::ports::{
    ...
    BeginReceiveOutcome, ClaimReceiveCommitPort, CleanupReceiveArtifactsPort,
    ClipboardEventRepositoryPort, ClockPort, CommitInboundReceivePort, CompletedReceiveArtifacts,
    FinalizeProvisionalReceivePort, GetEntryAttemptPort, InboundReceiveSettlement,
    NoEntryReceiveArtifacts, PartialReceiveArtifacts, PartialReceiveTerminal,
    ProvisionalReceiveAction, ReceiveArtifact, ReceiveItemRole, RequestReceiveCancellationOutcome,
...
pub struct ApplyInboundClipboardUseCase {
    ...
    /// Persistent FullMesh event-id replay lookup. Legacy channels omit an
    /// event id and continue through snapshot-hash dedup.
    event_repo: Option<Arc<dyn ClipboardEventRepositoryPort>>,
}
...
impl ApplyInboundClipboardUseCase {
    ...
    pub fn with_event_repository(
        mut self,
        repository: Arc<dyn ClipboardEventRepositoryPort>,
    ) -> Self {
        self.event_repo = Some(repository);
        self
    }
    ...
    async fn advance_active_register(
        ...
        let mobile_consumable = mobile_consumability
            ...
            .await;
        match register.advance(&state, mobile_consumable).await {
    ...
    async fn execute_internal(
        ...
        // FullMesh event-id replay guard. This query intentionally runs under
        // the same identity lock as the content commit: direct + relay racing
        // paths carrying one event cannot both pass the durable check before
        // the first path persists its event row.
        if let (Some(event_id), Some(event_repo)) =
            (input.event_id.as_ref(), self.event_repo.as_ref())
        {
            let replay_check_at_ms = match self.receive_attempts.as_ref() {
                Some(attempts) => attempts.clock.now_ms(),
                None => snapshot.ts_ms.max(0),
            };
            if event_repo
                .has_recent_event(event_id, replay_check_at_ms)
                .await
                .map_err(|error| ApplyInboundError::DedupQuery(error.to_string()))?
            {
                let existing_entry_id = self
                    ...
                    .await
                    .map_err(|error| ApplyInboundError::DedupQuery(error.to_string()))?;
                return Ok(match existing_entry_id {
                    Some(existing_entry_id) => ApplyOutcome::DuplicateSkipped {
                        snapshot_hash: input.snapshot_hash,
                        existing_entry_id,
                    },
                    None => ApplyOutcome::ReplaySkipped {
        ...
        if !is_partial {
            ...
            let should_write_clipboard = self
                .advance_active_register(
                    input.snapshot_hash.clone(),
                    entry_id.clone(),
                    input.from_device,
            ...
            if should_write_clipboard {
                ...
                debug!(entry_id = %entry_id, "inbound: stale LWW event persisted to history without overwriting the system clipboard");
                drop(snapshot_for_write);
            }
        } else {
            info!(
