...
use uc_core::ports::{
    ...
    ClipboardEventRepositoryPort, ClockPort, CommitInboundReceivePort, EntryReceiveAttempt,
    FinalizeProvisionalReceivePort, GetEntryAttemptPort, InboundReceiveCommitError,
    InboundReceiveSettlement, ProvisionalReceiveAction, ProvisionalReceiveError, PublishLogError,
    PublishPhase, ReceiveItemRole, RecordDirectoryPublishPort, RequestReceiveCancellationOutcome,
    ...
    let use_case = ApplyInboundClipboardUseCase::store_only_pull(StoreOnlyPullDeps {
        common: InboundApplyCommonDeps {
            ...
            entry_identity_coordinator: Arc::new(
                crate::clipboard::entry_identity::EntryIdentityCoordinator::new(),
            ),
        },
    });
...
struct ReplayEventRepository;

#[async_trait]
impl ClipboardEventRepositoryPort for ReplayEventRepository {
    async fn get_representation(
        &self,
        _id: &EventId,
        _representation_id: &str,
    ) -> Result<ObservedClipboardRepresentation> {
        anyhow::bail!("representation lookup is not part of this test")
    }

    async fn get_source_device(&self, _event_id: &EventId) -> Result<Option<DeviceId>> {
        Ok(None)
    }

    async fn has_recent_event(&self, _event_id: &EventId, _now_ms: i64) -> Result<bool> {
        Ok(true)
    }
}

#[tokio::test]
async fn persistent_event_tombstone_skips_replay_after_history_deletion() {
    let (mut input, _) = fixture_input("replayed after deletion");
    let event_id = EventId::new();
    input.event_id = Some(event_id.clone());
    let mut repo = MockEntryRepo::new();
    repo.expect_find_entry_id_by_snapshot_hash()
        .times(1)
        .returning(|_| Ok(None));
    ...
    let use_case =
        build(repo, capture, write).with_event_repository(Arc::new(ReplayEventRepository));

    assert_eq!(
        use_case.execute(input).await.expect("replay decision"),
