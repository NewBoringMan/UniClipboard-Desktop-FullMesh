...
impl ExistingLocalEntryDeliveryRunner for ExistingLocalEntryDelivery {
    async fn deliver(
        ...
        let mime_representations = snapshot
            .representations
            .iter()
            .map(
                |representation| uc_core::clipboard::ClipboardMimeRepresentation {
            ...
            .collect();
        let payload_references = blob_refs
            .iter()
            .map(|reference| uc_core::clipboard::ClipboardPayloadReference {
