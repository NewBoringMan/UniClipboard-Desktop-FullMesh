...
/// Transport that delivered one clipboard event. Unlike
/// [`ConnectionChannel`], this includes the asynchronous encrypted mailbox,
/// which is a delivery path rather than a live peer connection.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ClipboardDeliveryTransport {
    Direct,
    Relay,
    Mailbox,
    Unknown,
...
impl From<ConnectionChannel> for ClipboardDeliveryTransport {
    fn from(channel: ConnectionChannel) -> Self {
        match channel {
            ConnectionChannel::Direct => Self::Direct,
            ConnectionChannel::Relay => Self::Relay,
            ConnectionChannel::Offline | ConnectionChannel::Unknown => Self::Unknown,
        }
    }
}
...
pub struct InboundClipboard {
    ...
    /// Path that delivered this event, including the asynchronous mailbox.
    pub transport: ClipboardDeliveryTransport,
