...
use uc_application::facade::{
    ...
    InboundClipboardApplyPort, MailboxClipboardReceiver, MemberRosterDeps, MemberRosterFacade,
    MembershipConnectivityDeps, MembershipConvergenceDeps, MergedClipboardReceiver,
    SpaceAdmissionDeps, SpaceApplicationRuntime, SpaceConvergenceAssembly, SpaceConvergenceDeps,
    SpaceFacade, SpaceFacadeDeps, SpaceSessionDeps, SpaceTransitionDeps, TransferHostEvent,
    UpgradeFacade, UpgradeFacadeDeps, WorkspaceConvergenceDeps,
};
use uc_application::facade::{
...
use uc_core::ports::{
    ...
    ClipboardMailboxPort, ClipboardReceiverPort, ConnectionChannelPort, LocalIdentityPort,
    PresencePort,
};
...
pub async fn build_sync_engine_assembly(
    ...
    let iroh_clipboard_receiver: Arc<dyn ClipboardReceiverPort> = clipboard_receiver;
    let clipboard_receiver: Arc<dyn ClipboardReceiverPort> = if let Some(mailbox) = mailbox.clone()
    {
        let mailbox_receiver: Arc<dyn ClipboardReceiverPort> =
            Arc::new(MailboxClipboardReceiver::start(
                mailbox,
                Arc::clone(&deps.security.transfer_cipher),
                Arc::clone(&deps.device.device_identity),
            ));
        Arc::new(MergedClipboardReceiver::new(vec![
            iroh_clipboard_receiver,
            mailbox_receiver,
        ]))
    } else {
        iroh_clipboard_receiver
    ...
    let pull_store_apply: Arc<dyn InboundClipboardApplyPort> = Arc::new(
        ...
        .with_event_repository(Arc::clone(&shared.clipboard_event_reader_repo)),
    );
