...
use uc_core::ports::ClipboardMailboxPort;
...
pub async fn build_daemon_lifecycle(
    ...
    mailbox_service: Option<(String, crate::SecretString)>,
    ...
    let mailbox: Option<Arc<dyn ClipboardMailboxPort>> = mailbox_service
        .map(|(base_url, token)| {
            uc_infra::network::HttpClipboardMailbox::new(&base_url, token.expose().to_owned())
                .map(|mailbox| Arc::new(mailbox) as Arc<dyn ClipboardMailboxPort>)
        })
        .transpose()
        .map_err(|error| anyhow::anyhow!("mailbox configuration is invalid: {error}"))?;
