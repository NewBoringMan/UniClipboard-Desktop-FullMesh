...
mod switch_tests {
    ...
    fn reverting_revision_migration_removes_runtime_revision_triggers() {
        ...
        // Revert FullMesh event replay state, retired legacy cleanup, then
        // the database-revision migration whose triggers this test owns.
    ...
    fn retired_legacy_upgrade_records_are_cleared_by_migration() {
        ...
        // FullMesh event replay state is newer than the cleanup migration.
        connection.revert_last_migration(MIGRATIONS).unwrap();
        connection.revert_last_migration(MIGRATIONS).unwrap();
        diesel::sql_query(
