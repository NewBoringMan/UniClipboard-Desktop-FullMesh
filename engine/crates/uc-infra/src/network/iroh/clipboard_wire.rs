...
use uc_core::clipboard::{
    ClipboardEventEnvelope, ClipboardMimeRepresentation, ClipboardPayloadReference,
};
...
#[derive(Serialize, Deserialize, Debug)]
struct WireMimeRepresentationV1 {
    mime_type: String,
    size_bytes: u64,
}

#[derive(Serialize, Deserialize, Debug)]
struct WirePayloadReferenceV1 {
    blob_id: String,
...
struct WireClipboardEventV1 {
    ...
    expires_at_ms: i64,
    content_hash: String,
    mime_representations: Vec<WireMimeRepresentationV1>,
    payload_references: Vec<WirePayloadReferenceV1>,
    target_device_ids: Vec<String>,
    hop_count: u8,
    schema_version: u16,
}

/// FullMesh schema shared by v4 (ordinary payload) and v5 (directory
/// manifest payload). v1-v3 remain decode-only compatibility formats.
#[derive(Serialize, Deserialize, Debug)]
struct WireHeaderV4 {
    version: u8,
    snapshot_hash: String,
...
pub enum WireEncodeError {
    ...
    #[error("FullMesh event envelope is missing or invalid")]
    InvalidEnvelope,
}
...
pub enum WireDecodeError {
    ...
    #[error("stream io: {0}")]
    Io(std::io::Error),
    #[error("FullMesh event envelope is invalid")]
    InvalidEnvelope,
}
...
pub fn encode_header(header: &ClipboardHeader) -> Result<Vec<u8>, WireEncodeError> {
    let bytes = match header.version {
        2 | 3 => postcard::to_allocvec(&WireHeaderV2 {
        ...
        ClipboardHeader::CURRENT_VERSION | ClipboardHeader::DIRECTORY_VERSION => {
            let event = header
                .event
                .as_ref()
                .ok_or(WireEncodeError::InvalidEnvelope)?;
            event
                .validate()
                .map_err(|_| WireEncodeError::InvalidEnvelope)?;
            postcard::to_allocvec(&WireHeaderV4 {
                version: header.version,
                snapshot_hash: header.snapshot_hash.clone(),
                captured_at_ms: header.captured_at_ms,
                origin_device_id: header.origin_device_id.clone(),
...
pub fn decode_header(bytes: &[u8]) -> Result<ClipboardHeader, WireDecodeError> {
    ...
    match version {
        ...
        4 | 5 => {
            let wire: WireHeaderV4 = postcard::from_bytes(bytes)?;
            let event = envelope_from_wire(wire.event)?;
            if event.content_hash != wire.snapshot_hash
                || event.origin_device_id.as_str() != wire.origin_device_id
            {
                return Err(WireDecodeError::InvalidEnvelope);
            }
            Ok(ClipboardHeader {
                version: wire.version,
                snapshot_hash: wire.snapshot_hash,
...
fn envelope_to_wire(event: &ClipboardEventEnvelope) -> WireClipboardEventV1 {
    WireClipboardEventV1 {
        event_id: event.event_id.to_string(),
        origin_device_id: event.origin_device_id.as_str().to_owned(),
        origin_sequence: event.origin_sequence,
        created_at_ms: event.created_at_ms,
        expires_at_ms: event.expires_at_ms,
        content_hash: event.content_hash.clone(),
        mime_representations: event
            .mime_representations
            .iter()
            .map(|representation| WireMimeRepresentationV1 {
                mime_type: representation.mime_type.clone(),
                size_bytes: representation.size_bytes,
            })
            .collect(),
        payload_references: event
            .payload_references
            .iter()
            .map(|reference| WirePayloadReferenceV1 {
                blob_id: reference.blob_id.clone(),
                content_hash: reference.content_hash.clone(),
                size_bytes: reference.size_bytes,
            })
            .collect(),
        target_device_ids: event
            .target_device_ids
            .iter()
...
fn envelope_from_wire(
    event: WireClipboardEventV1,
) -> Result<ClipboardEventEnvelope, WireDecodeError> {
    let origin_device_id =
        DeviceId::try_new(&event.origin_device_id).ok_or(WireDecodeError::InvalidEnvelope)?;
    let target_device_ids = event
        .target_device_ids
        .into_iter()
        .map(|device| DeviceId::try_new(device).ok_or(WireDecodeError::InvalidEnvelope))
        .collect::<Result<Vec<_>, _>>()?;
    let envelope = ClipboardEventEnvelope {
        event_id: EventId::from_string(event.event_id),
        origin_device_id,
        origin_sequence: event.origin_sequence,
        created_at_ms: event.created_at_ms,
        expires_at_ms: event.expires_at_ms,
        ...
        mime_representations: event
            .mime_representations
            .into_iter()
            .map(|representation| ClipboardMimeRepresentation {
        ...
        payload_references: event
            .payload_references
            .into_iter()
            .map(|reference| ClipboardPayloadReference {
    ...
    envelope
        .validate()
        .map_err(|_| WireDecodeError::InvalidEnvelope)?;
    Ok(envelope)
}
...
mod tests {
    ...
    /// 8. Legacy v2 round-trip — encode a header with a flow_id and confirm it
    /// survives the decode boundary intact (i.e. cross-device correlation
    /// can rely on the field).
    #[tokio::test]
    async fn legacy_v2_round_trip_preserves_flow_id_without_event() {
    ...
    #[tokio::test]
    async fn legacy_v3_round_trip_remains_decode_compatible() {
        let mut header = sample_header();
        header.version = 3;
    ...
    #[tokio::test]
    async fn fullmesh_v4_round_trip_preserves_event_identity() {
        let header = sample_header();
        let bytes = encode_header(&header).unwrap();
        let decoded = decode_header(&bytes).unwrap();

        assert_eq!(decoded.version, ClipboardHeader::CURRENT_VERSION);
        assert_eq!(decoded.event, header.event);
    }

    #[tokio::test]
    async fn directory_header_round_trip_uses_v5_without_changing_default_version() {
        let mut header = sample_header();
        ...
        assert_eq!(decoded.flow_id, header.flow_id);
        assert_eq!(ClipboardHeader::CURRENT_VERSION, 4);
    }

    #[test]
    fn fullmesh_decode_rejects_header_event_origin_or_hash_tampering() {
        let header = sample_header();
        let event = header.event.as_ref().expect("sample event");
        let mut wire = WireHeaderV4 {
            version: ClipboardHeader::CURRENT_VERSION,
            snapshot_hash: header.snapshot_hash.clone(),
            captured_at_ms: header.captured_at_ms,
            origin_device_id: "tampered-origin".to_owned(),
            origin_device_name: header.origin_device_name.clone(),
            payload_version: header.payload_version,
            flow_id: header.flow_id.clone(),
            event: envelope_to_wire(event),
        };
        let origin_tampered = postcard::to_allocvec(&wire).unwrap();
        assert!(matches!(
            decode_header(&origin_tampered),
            Err(WireDecodeError::InvalidEnvelope)
        ));

        wire.origin_device_id = event.origin_device_id.as_str().to_owned();
        wire.snapshot_hash = "b".repeat(64);
        let hash_tampered = postcard::to_allocvec(&wire).unwrap();
        assert!(matches!(
            decode_header(&hash_tampered),
            Err(WireDecodeError::InvalidEnvelope)
        ));
    }
