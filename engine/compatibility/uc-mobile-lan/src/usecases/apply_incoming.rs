...
pub enum IncomingMobileClipEvent {
    ...
    ///
    /// The handler-generated `transfer_id` follows the provisional receive
    /// into SyncDoc apply, where the generic inbound attempt adopts and
    /// settles it. The handler already emitted `Started` and `Progress`.
    BufferFile {
        data_name: String,
        mime: String,
        staged: StagedFile,
...
/// Image and File branches retain the provisional transfer ID so SyncDoc
/// apply can adopt and settle the exact receive item.
struct BuiltSnapshot {
    snapshot: SystemClipboardSnapshot,
    transfer_id: Option<String>,
    receive_item_role: Option<ReceiveItemRole>,
...
struct BufferedFile {
    ...
    /// 内联到 image rep。
    staged: StagedFile,
    /// Protocol transfer ID generated by `PUT /file` or supplied as
    /// `upload_id`; SyncDoc uses it to adopt the provisional receive.
    transfer_id: String,
}
...
pub(crate) struct ApplyIncomingMobileClipUseCase {
    ...
    /// Optional file-transfer lifecycle facade used to journal provisional
    /// paths and settle the adopted transfer after SyncDoc apply.
...
impl ApplyIncomingMobileClipUseCase {
    ...
    pub(crate) async fn execute(
        ...
        match input.event {
            IncomingMobileClipEvent::BufferFile {
                ...
                if let Some(file_transfer) = self.file_transfer.as_ref() {
                    if let Err(error) = file_transfer
                        .record_provisional_receive_path(&transfer_id, &uri)
                        .await
                    {
            ...
            IncomingMobileClipEvent::SyncDoc {
                ...
                let dispatch_outcome = self
                    .dispatch_inbound(
                        ...
                        self.file_transfer
                            .as_ref()
                            .and_then(|_| transfer_id.clone().zip(receive_item_role)),
                    )
                    .await;
                let keep_staged_file = matches!(
                    (&dispatch_outcome, receive_item_role),
                    (
                        Ok(ApplyIncomingMobileClipOutcome::Applied { .. }),
                        Some(ReceiveItemRole::File)
                    )
                );
                if !keep_staged_file {
    ...
    /// Settle the Mobile transfer after SyncDoc apply. Entry ownership is
    /// already established by provisional adoption in the inbound attempt.
    /// Applied, resurfaced, and duplicate outcomes complete the received bytes;
    /// decode and application failures mark the transfer failed.
    ...
    async fn build_image_snapshot(
        ...
        Ok(BuiltSnapshot {
            ...
            receive_item_role: Some(ReceiveItemRole::Representation),
    ...
    fn build_file_snapshot(
        ...
        Ok(BuiltSnapshot {
            ...
            transfer_id: Some(transfer_id),
            receive_item_role: Some(ReceiveItemRole::File),
            staged_file: Some(staged_file),
        })
    }
    ...
    async fn dispatch_inbound(
        ...
        let input = ApplyInboundInput {
            from_device: pseudo_from,
            // The LAN compatibility transport predates FullMesh envelopes.
            // Its persistent content-hash path remains the deduplication key.
            event_id: None,
        ...
        Ok(match outcome {
            ...
            ApplyOutcome::ReplaySkipped { .. } => {
                // This compatibility transport deliberately supplies no
                // FullMesh event id, so the replay guard cannot normally
                // produce this outcome. Keep the boundary total and fail
                // closed if that contract changes underneath it.
                let reason =
                    "legacy LAN compatibility delivery unexpectedly entered the replay guard"
                        .to_string();
                warn!(%reason, "mobile_sync apply_incoming: incompatible replay outcome");
